...
Leaf::Leaf(int sym) : symbol(sym) {}

InternalNode::InternalNode(std::unique_ptr<Node> &&left, std::unique_ptr<Node> &&right) : leftChild(std::move(left)),
                                                                                          rightChild(std::move(right)) {}

CodeTree::CodeTree(InternalNode &&rt, int symbolLimit) : root(std::move(rt))
{
    if (symbolLimit < 2)
...
const vector<char> &CodeTree::getCode(int symbol) const
{
